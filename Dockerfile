...
ARG CRUNCHY_IMAGE_VERSION=3
ARG TIMESCALE_VERSION=2.10.3
ARG TIMESCALE_TOOLKIT_VERSION=1.17.0
...
RUN curl -sSL -o /etc/yum.repos.d/timescale_timescaledb.repo "https://packagecloud.io/install/repositories/timescale/timescaledb/config_file.repo?os=el&dist=8" && \
    ...
    microdnf --disablerepo=crunchypg${POSTGRES_MAJOR_VERSION} --disablerepo=pgdg-common --disablerepo=pgdg15 --disablerepo=ubi-8-baseos-rpms --disablerepo=ubi-8-appstream-rpms --setopt=install_weak_deps=0 update -y && \
