...
RUN curl -sSL -o /etc/yum.repos.d/timescale_timescaledb.repo "https://packagecloud.io/install/repositories/timescale/timescaledb/config_file.repo?os=el&dist=8" && \
    microdnf --disablerepo=crunchypg${POSTGRES_MAJOR_VERSION} --disablerepo=ubi-8-baseos --disablerepo=ubi-8-appstream --disablerepo=esquilax-private --setopt=install_weak_deps=0 update -y && \
    microdnf --disablerepo=crunchypg${POSTGRES_MAJOR_VERSION} --disablerepo=ubi-8-baseos --disablerepo=ubi-8-appstream --disablerepo=esquilax-private --setopt=install_weak_deps=0 install -y postgresql-unit-${POSTGRESQL_UNIT_VERSION} && \
    microdnf --disablerepo=crunchypg${POSTGRES_MAJOR_VERSION} --disablerepo=ubi-8-baseos --disablerepo=ubi-8-appstream --disablerepo=esquilax-private --setopt=install_weak_deps=0 install -y \
